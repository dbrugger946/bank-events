...
cat <<EOF > generated/lis-bank-events.sh
	...
	-topic bank-events --from-beginning \\
	--consumer-property ssl.truststore.location=truststore.jks
EOF
chmod +x generated/lis-bank-events.sh

cat <<EOF > generated/pro-bank-events.sh
...
EOF
chmod +x generated/pro-bank-events.sh
